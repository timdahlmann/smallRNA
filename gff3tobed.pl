...
=head2 DISCRIPTION
The script gff3tobed.pl converts gff3 in bed format. For each entry inside the gff3 file it creates a gene entry and a corresponding features 
...
=head5 COPYRIGHT (C)
The program gff3tobed.pl is free software: you can redistribute it and/or modify it under the terms of the 
...
##### hardcoded option #####
my $inputfile = "Bt65_Chr_FINAL_Feb2021_repeatmasker_renamed.gff3";	# hardcoded input file, if you don't want to use the shell just enter the file name here
...
if ($num_args > 1) {
  print	"\nERROR: wrong number of arguments!\nUsage: gff3tobed.pl inputfile.gff or use hardcoded option\nType gff3tobed.pl -h for further details.\n\n";
  exit;
}
...
foreach my $elem (@gff_input) { 
	...
	if (($elem =~ /^[^#]/) and ($elem =~ /Name=(Cnl1_.*)$/)) {
		...
		my $name = $1;
		my $strand = "$cols[6]";
		my $bed = "$contig\t$start\t$stop\t$name\t0\t$strand\n";
		push (@daten, $bed);
		}
}		
...
open OUTPUT, "> $outfile";	# open a fasta file
